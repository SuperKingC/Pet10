// 衣柜素材 v3：网格服装特写 + 三件叠穿件（帽/巾/包）
// —— 帽子/小包：画布散件直切（干净）。
// —— 围巾：从围巾狗上按「三角巾+小花」连通填充切出（只挡棕毛，深色描边和白格保留，舌头/胸毛被棕毛边界挡住）。
// —— 连帽衫/背带裤/小裙子：颜色分割（衣服专属色板 + 非棕毛连通生长），只留衣服不留毛。
// —— 雨衣/睡衣：黄色/浅紫与毛发色不可分，保持矩形裁窗（边缘少量毛，卡面尺寸可接受）。
// 叠穿件在 app 内按 wardrobeModel.ts 的定位元数据（与下表 cx/ty/w 一致）叠到原装立绘上。
...
// —— 服装标定（256 画布坐标，逐像素采样定稿）——
const overlaySuits = {
  // 帽子：紫色贝雷帽散件，戴在头顶
  hat: { rect: { src: 'purple-beret', box: { x1: 98, y1: 202, x2: 156, y2: 243 } }, cx: 218, ty: 52, w: 186 },
  // 围巾：三角巾+小花（连通填充只挡棕毛，深描边/白格保留；舌头与胸毛被棕毛挡在外面）
  scarf: {
    fill: {
      src: 'scarf',
      crop: { x1: 84, y1: 184, x2: 172, y2: 214 },
      seedBoxes: [[112, 188, 140, 202]],
      mode: 'block-tan'
    },
    cx: 218, ty: 350, w: 252
  },
