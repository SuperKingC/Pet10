// 小多利走路两帧 v2 出件（大张嘴咬球姿势源图）：白底去底（边界泛洪+最大连通成分）→
// 两帧统一画布且脚底基线对齐 → PNG8 出件 public/wardrobe/xiaoduoli-walk-{a,b}-v2.png
// 运行：node miniapp/tools/make-walk-frames-v2.mjs
...
const strideCut = await cutSource('xiaoduoli-walk-stride-source-v2.png')
const passCut = await cutSource('xiaoduoli-walk-pass-source-v2.png')
...
for (const [name, buf, meta] of [
  ['xiaoduoli-walk-a-v2.png', strideResized, strideMeta],
  ['xiaoduoli-walk-b-v2.png', passResized, passMeta],
]) {
  ...
  frames.push({ file: name, bytes: out.byteLength, canvas: canvasW + 'x' + FRAME_HEIGHT_PX, contentWidth: meta.width, left, top })
}
