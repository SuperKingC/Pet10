...
const COMMON_TAIL2 = '衣领最高只能到下巴正下方（绝对不能包住脸颊两侧、不能碰到嘴巴和舌头），下摆停在爪子上方（绝对不能盖住爪子和脚背）。'
const COMMON_TAIL = '。' + COMMON_TAIL2 + '衣服保持手绘水彩上色、细深棕色描边。纯白色背景，没有阴影，没有文字。柔和的手绘水彩儿童绘本风格，笔触柔软。'

const GARMENTS = {
  hoodie: `${COMMON_HEAD}一件奶油绿色的连帽卫衣（帽兜翻在颈后垂在背后、绝不盖住头顶，白色抽绳垂在胸前，宽松版型，袖子顺身体两侧垂下，衣长盖过肚子），衣服颜色样式不变。${COMMON_TAIL}`,
  overalls: `${COMMON_HEAD}一条蓝色牛仔背带裤（两条肩带从胸前完整向上延伸、搭在两肩上、肩带连续不断不悬空，胸前口袋带白色小骨头图案、棕色圆纽扣，裤腿宽松盖过大腿），衣服颜色样式不变。${COMMON_TAIL}`,
