...
describe('miniapp ui presentation rules', () => {
  ...
  it('presents MBTI, contact, about and avatar editing through the shared modal', () => {
    ...
    // 档案文案抽到 xiaoduoliProfile（与名片弹窗共用），MeView 只持弹窗接线
    const profile = fs.readFileSync(path.resolve(__dirname, 'xiaoduoliProfile.ts'), 'utf8')
    expect(profile).toContain('仅此一只')
    expect(profile).toContain('老实巴交')
    expect(profile).toContain('等妈妈回家')
    expect(meView).toContain('XIAODUOLI_PROFILE_LINES')
  ...
  it('shows a locked box scene and unlock button after a friend accepts the invitation', () => {
    ...
    expect(scene).toContain('xiaoduoli-box-v2.png')
    ...
    // 旧版干净纸箱素材已退役
    expect(scene).not.toContain("require('../../assets/nest/xiaoduoli-box.png')")
    ...
    expect(styles).toContain('@keyframes xiaoduoli-scrap')
    expect(styles).toContain('@keyframes xiaoduoli-box-intro')
    expect(styles).not.toContain('@keyframes xiaoduoli-peek')
    expect(styles).toContain('@media (prefers-reduced-motion: reduce)')
    expect(manifest).toContain('nest/xiaoduoli-box-v2.png')
