...
export function FriendSetupScreen({ session, onChanged }: FriendSetupScreenProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      else await friendshipApi.sendRequest(username)
      onChanged()
    } catch (requestError) {
    ...
    <main className="onboarding-shell">
      <div className="onboarding-card friend-setup-card">
        ...
        {session.status === 'unbound' && (
          <>
            <p>输入朋友的用户名。对方接受后，你们会共同拥有一只小多利。</p>
            <label>
              朋友用户名
              <input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="例如：xiaoming" />
            </label>
          </>
        )}
        {error && <div className="form-error">{error}</div>}
        <button className="primary-action" disabled={busy || (!incoming && session.status === 'unbound' && username.length < 3)} onClick={() => void submit()}>
