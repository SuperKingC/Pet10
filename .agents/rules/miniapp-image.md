...
2. 落库前：只做**一次**有损压缩，统一走 `miniapp/src/services/imageCompression.ts` 的 `compressImageToDataUrl`：
   ...
   - 必须传 `compressedWidth`，按实际展示宽度取档：日记/纪念日照片 `[1080, 900, 720]`，头像 `[640, 480, 360]`；720 仍超限的极繁照片依次回退 `[540, 420]`（2026-08 实测个别照片 720px q80 仍超 300_000 字符导致无法上传）。
...
- 包体紧张时**不得用色板降深（如 256→96 色）、关抖动、降 JPEG 质量等方式换取空间**——水彩/渐变素材会出现色阶断裂与色彩劣化，用户可感知。
- 允许的腾空间手段（不影响色彩还原）：按显示尺寸重采样（缩尺寸）、裁切留白、COS 按需下载、分包、删除无引用资源。
- 既有素材如已发生此类压色，验收时应恢复；「压色前 vs 压色后」需放大目检色阶过渡。
