...
- 微信登录页支持用户主动选择微信头像并填写微信昵称；头像临时文件会转为受限大小的图片数据后保存，未选择时继续使用“微信用户”和默认头像。
...
- 无好友时，小窝页显示“小窝”标题和固定副标题，PWA 同源的小窝背景右侧纵向展示放大的主要快捷入口，不显示重复的邀请引导标题或正常恢复提示。
...
- [x] 微信登录可主动选择头像和填写昵称，资料接口保留用户确认的资料；
