...
- 小窝右上「衣柜」图标打开面板：顶部「试衣间」场景——小多利立绘（点击目录即时换装）直接站在衣柜内景大图的地板上（内景下沿渐变融入面板底色），叠加呼吸聚光锥、地板椭圆光影、两颗闪烁星星与脚下套装名牌（弹入），内景大图缓慢漂移（14s 往复）；场景下方为全宽「今日默契换装」细条卡（虚线尾巴指向小多利，连胜火焰+当前/最高连胜chip）；下半 3 列目录网格——木质挂杆（木纹拼贴+两端圆头球，卡片顶部带小挂钩）下的已解锁高亮点选、未解锁灰置加锁印+条件文案+途径徽章（任务/等级/暗号/默契/睡觉），选中卡片摇摆后金光呼吸；底部「保存装扮」（PUT equipped）与「就选它，提交默契」（POST match）双按钮。
