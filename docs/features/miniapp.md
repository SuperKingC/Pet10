...
- 活跃态小窝整页一屏不滚动（排版紧凑化，底部「今日默契换装」横卡已移除，默契状态收进衣柜面板内）：场景卡上半是 439px 高室内场景——背景 `room-background-v5.jpg`（AI 以原版房间为参考图按 3:2 场景比例重新生成的横构图：窗/「Happy Day」牌/矮书架齐全，地毯缩为中心椭圆、四周露地板；720×480 40KB）`widthFix` 通栏铺满，显示高恰为 439rpx 与场景等高、无空带无两侧留白；小多利立绘 200×236 绝对定位居中偏下（bottom 14px）踩在地毯上；说话飘字气泡单行、居中悬在头顶上方（top 136px：牌子底与狗头顶之间，不挡木牌；气泡常驻不随换文案重挂载、只替换文本，背景用 aspectFill 定高替代 widthFix，规避微信 widthFix 图在兄弟节点 setData 时重测量导致的背景闪动；`xiaoduoliSpeech.ts` 纯函数出文案：心情引擎状态优先，饿了/累/不舒服/困等需求态次之，其余按序轮播闲聊池，6s 换一句，换句重新弹入 + 轻微浮动，`prefers-reduced-motion` 静止）；左下「小多利 · 名片」白卡双线描边入口点击打开名片弹窗；右侧快捷入口（衣柜/照片墙/任务）从「记忆」按钮下方起排（top 84px、图标 92×96、间距 16px）。场景卡下半是紧凑状态区（成长经验 + 饱食/心情/精力/健康 2×2，原「开心地陪着你们」状态行已删；进度条加粗 16rpx，凹槽底 + 填充顶部高光/底部投影、四项状态各自同色系渐变，宽度变化 0.4s 过渡）；情绪激动时场景上半部飘 b 站式弹幕（`xiaoduoliDanmaku.ts` 纯函数触发规则：心情引擎 happy 档或 mood≥75 → 7s 一条、喂食/玩耍等状态刷新连发 3 条；content 档或 mood≥45 → 12s 一条；bored/sulky/angry 或饿/累/病/困告急不飘；同屏上限 5 条，白字描影 transform 匀速从右向左漂过，漂出由定时器移除，`prefers-reduced-motion` 不启用）。下方照顾栏、贡献榜收紧内边距，主流机型合计恰好一屏，矮机型由页面滚动兜底。
...
- 小窝自动「闲逛」与玩耍「叼娃娃」行为幕：站立态每 24~54s（`nextWanderDelayMs` 随机）自动出发闲逛一趟——AI 生成的两帧侧身步态（腾空尾高 `xiaoduoli-walk-a-v1` / 触地尾低 `xiaoduoli-walk-b-v1`，统一画布 468×300 脚底基线对齐，显示 234×150）以 0.42s step-end 交替成小跑循环，叠 0.42s 颠步+1.2° 轻摇，travel 一镜到底走中心→右缘→翻面（scaleX -1）→走回中心（7s = `NEST_PET_WANDER_MS`）；照顾栏点玩耍立即（睡觉中也会跳起来）出发「叼娃娃」分镜（9s = `NEST_PET_FETCH_MS`）：走向左缘→走出画面淡出→换向叼着红领巾绒毛娃娃（`xiaoduoli-doll-v1.png` 46×64，挂 travel 层跟随翻面）走回中心→娃娃抛落弹地压扁回弹→开心蹦两下，气泡定格「把娃娃叼回来啦！」，娃娃停留后淡出；喂食/清洁不打断行进分镜，玩耍可打断闲逛改去叼娃；素材三张齐了才播分镜（未就绪静默保持站姿），`wardrobeAssetLoader` 下载+索引写回已串行化（并发 ensureFile 不再互相覆盖索引条目），全部动效带 `prefers-reduced-motion` 降级（分镜不播、娃娃不出现）。
...
- 小窝右上「衣柜」图标打开面板：顶部「华丽舞台」试衣间场景——AI 重生成的衣帽间内景（金边双层圆台舞台+吊灯聚光，`wardrobe-interior-v3.jpg` 1152×768，COS 按需下载，未上线时回退 v2 内景），小多利立绘（点选即时试穿）按各自主体的宽高比换算宽度、脚踩在圆台台面（显示 870×580rpx，脚位约 y=420rpx），叠加呼吸柔光、两颗闪烁星星与压在舞台前沿的套装名牌（弹入）；内景缓慢漂移（14s 往复）。场景下方为全宽「今日默契换装」细条卡（连胜火焰+当前/最高连胜 chip）。
- **按类别穿戴（每类一件）**：目录分「🐾 主体服装（选一件）」（原装/连帽衫/背带裤/小裙子/雨衣/睡衣）与「🎀 配饰（可叠穿）」（帽子/围巾/小包，点选佩戴、再点摘下）两区；预览按 pieces 多层合成——主体整身立绘 + 配饰按标定叠加（原装用 `OUTFIT_LAYER_STYLE`，其余主体服装由 `miniapp/tools/calibrate-body-overlays.mjs` 按同姿势紧裁立绘线性换算出 `BODY_OVERLAY_STYLE`，改素材必须重跑标定）；立绘走 widthFix flow 模式（容器宽度=`suitDisplayWidth(key, 高度)`，图盒=容器盒，叠加百分比与图对齐），小窝场景立绘（`PetStatusCard`）同口径展示双方穿戴。`GET /wardrobe` 返回 `outfit`（body/hat/scarf/bag）；`PUT /wardrobe` 接 `{ outfit }`（兼容旧 `{ itemKey }`=只换主体），`pet_wardrobe.equipped` 列以 JSON 存整套（TEXT 列免迁移，旧单 key 数据自动解析兼容）；未解锁件整套拒绝 409 `wardrobe_locked`。默契换装仍按**主体服装**提交与比对。
...
- 套装素材分发：主包内置原装立绘 + 三件叠穿件（帽/巾/包，`miniapp/src/assets/wardrobe/` 下 `outfit-hat-v1.png`、`outfit-scarf-v1.png`、`outfit-bag-v1.png`，包体红线约束）；主体服装的特写图标与整套立绘发布在 `public/wardrobe/`（随 `upload:static` 上 COS），小程序经 `TARO_WARDROBE_ASSET_BASE_URL`（默认取塔罗静态目录下的 `wardrobe/`，可环境变量覆盖）按需下载，落 `USER_DATA_PATH` 本地缓存（索引存 storage，`wardrobeAssetLoader.ts` 注入式核心 + `wardrobeSuitAssets.ts` 运行时绑定，icon/display 双文件口径）。素材未就绪/下载失败时网格卡与预览显示「云端准备中」并回退原装立绘，不阻塞其余功能。
- 服务端纯规则：`outfitMatchRules.ts`（双方齐才结算、连胜/最高连胜）、`photoWallRules.ts`（36 张上限、只淘汰手动照、caption 归一）、`codewordStreak.ts`（连胜回扫与每满 7 天发卡）。小程序纯函数：`photoWallModel.ts`（来源徽章、模板卡判定、两列拆分、日期文案）、`wardrobeModel.ts`（叠穿分类与定位元数据、随包套装表、途径徽章、默契状态文案）。
...
- `miniapp/src/features/main/MiniappPetCardModal.tsx`
...
| `wardrobe/xiaoduoli-walk-a-v1.png`（COS） | 468×300 | 64 KB | 闲逛/叼娃步态帧 A：腾空迈步、尾巴高扬（AI 参考生成+两帧统一画布底对齐，显示 234×150） |
...
| `room-background-v5.jpg` | 720×480 | 40 KB | 小窝宠物场景背景（AI 以原版房间为参考按 3:2 场景比例重生成：窗/「Happy Day」牌/矮书架齐全，地毯缩为中心椭圆四周露地板；显示高 439rpx 与场景等高无空带；v2 丢牌子、v3 裁横带、v4 通栏地毯版均弃用） |
