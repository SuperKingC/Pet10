...
小程序首页在登录后准备服务端邀请 token，并通过 `Button openType="share"` 分享 `/pages/invite/invite?token=...`。已有小窝的用户仍可继续邀请其他好友；分享后的下一次邀请使用新的 token。
...
当用户打开自己发出的邀请时，邀请页不展示接受或拒绝操作。客户端识别邀请人与当前用户相同后，使用统一提示队列显示约 1 秒的轻提示，清理待处理邀请并自动返回首页。
