...
- 可灵参数（2026-09-22 实测）：`resolution: '720p'|'1080p'`（分辨率/清晰度档位由模型选择决定，std≈$0.63/5s、pro≈$0.84/5s）；`duration: 5|10`（数字，10 秒费用翻倍，实测 10s 成片 10.04s）；`audio: true`（成片带 aac 音轨）；比例跟随首帧图（方图首帧出 960×960），纯文生视频为方画幅；512px 首帧被拒、1024px 通过。出图参数：图片任务支持 `aspectRatio` 七档（1:1 / 16:9 / 9:16 / 4:3 / 3:4 / 3:2 / 2:3，其中 16:9/9:16/4:3 实测透传出图 1280×720、720×1280、1152×864）、`imageSize`（1K/2K，仅 openai 系模型生效，页面默认 1K）、`count`（1~4 张，一次提交并行出图，部分失败交付成功部分）。提示词优化由 `IMAGE_PROMPT_MODEL`（默认 `openai/gpt-5.4-mini`，单次约 $0.0007）驱动。
