...
- 对外接口（`server/src/http/imageRoutes.ts`）：`POST /api/images/generations` 同步生图（阻塞直到出图，仅图片模型）；`POST /api/images/tasks` 提交异步任务（提交即返回 202）；`GET /api/images/tasks/:id` 轮询任务状态与结果；`GET /api/images/models` 启用模型目录（公开只读）；`GET /api/images/quota` 剩余额度（鉴权但不消耗额度，图/视频共用额度池）；`POST /api/images/optimize` 提示词优化（独立宽松限额 6/分钟、30/天，调文本模型按图/视频两套规则改写，不占生成次数）。全部用 `Authorization: Bearer <生图邀请码>` 鉴权，成功响应为 OpenAI images 兼容的 `data` 数组加 `durationMs`。
...
- 可灵参数（2026-09-22 实测）：`resolution: '720p'|'1080p'`（分辨率/清晰度档位由模型选择决定，std≈$0.63/5s、pro≈$0.84/5s）；`duration: 5|10`（数字，10 秒费用翻倍，实测 10s 成片 10.04s）；`audio: true`（成片带 aac 音轨）；比例跟随首帧图（方图首帧出 960×960），纯文生视频为方画幅；512px 首帧被拒、1024px 通过。出图参数：图片任务支持 `aspectRatio`（1:1/2:3/3:2）、`imageSize`（1K/2K，仅 openai 系模型生效）、`count`（1~4 张，一次提交并行出图、消耗一个额度，部分失败交付成功部分）。提示词优化由 `IMAGE_PROMPT_MODEL`（默认 `openai/gpt-5.4-mini`，单次约 $0.0007）驱动。
- 浏览器测试页：`https://api.pet10kk.com/image-playground`（`server/src/http/imagePlaygroundRoutes.ts`，页面内联在 TS 里随 `server:build` 一起产出）。从模型下拉选择图/视频模型（表单字段按类型联动：图=比例/尺寸/数量+参考图，视频=分辨率/时长/声音+首帧图），「✨优化提示词」一键改写，页面顶部实时显示剩余生成额度，任务卡片带计时、多任务并行轮询，图片/视频结果内联渲染（一次任务多张图以网格展示）。页面本身不含任何密钥，邀请码只保存在使用者浏览器的 localStorage；页面带 `noindex`。任务结果只存内存，保留 30 分钟、上限 30 个任务，重启即清。
