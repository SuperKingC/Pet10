...
flowchart TD
  ...
  R --> F
  G -->|"否"| H["登录或好友绑定"]
- `src/startupAssets.ts`：集中维护主应用关键图片清单，并在初始已有会话或登录成功后非阻塞预载。
  G -->|"是"| E
```
...
会话列表预载房间时，每个房间只允许一个 bootstrap 在途请求；并发调用复用同一个 Promise，成功加载过的房间再次进入时不重复请求。

底栏图标位于 `public/navigation/`，以 128×128 透明 PNG 保存，在页面中按视口使用 `clamp()` 自适应显示，普通图标约为 34–42px、脚印圆约为 50–58px。图标来自设计稿切图，已去除外部浅色画布。浅蓝色导航底板使用 `public/navigation/tab-bar-background.png`，继续从 COS 加载，并以真实图片层铺在图标和文字后方，避免 iOS 主屏独立模式对 CSS 伪元素背景的兼容差异。
...
- Service Worker 应用壳必须预缓存底部导航浅蓝背景图；更新该清单时必须升级缓存版本，使已安装的主屏入口删除旧应用壳并重新获取背景资源。
- 首次会话请求最多等待 8 秒；超时不得删除登录令牌，必须显示重新连接入口。
- 登录页完成认证后必须补跑主应用关键图片预载，不能依赖模块首次执行时已经存在访问令牌。
- 房间 bootstrap 失败时不能写入成功缓存，后续进入必须能够重新请求。
...
- [ ] 从登录页完成认证后进入主应用时，宠物、房间背景、底部导航和“我的”固定图标已开始预载。
- [ ] 会话列表预载和进入同一房间并发发生时只请求一次，成功后再次进入不重复请求。
