...
describe('GitHub workflows', () => {
  ...
  it('requires the production environment and calls fixed deploy scripts', async () => {
    ...
    expect(workflow).not.toContain("ref: ${{ inputs.commit || 'main' }}")
    expect(workflow).toContain('REQUESTED_REVISION="${{ inputs.commit }}"')
    expect(workflow).toContain('git rev-parse --verify "${REQUESTED_REVISION}^{commit}"')
