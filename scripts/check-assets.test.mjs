...
describe('asset checks', () => {
  it('reports runtime assets and source-only assets separately', async () => {
    ...
    expect(report.assets.filter((asset) => asset.category === 'source-only')).toHaveLength(24)
    expect(report.assets.some((asset) => asset.path.startsWith('design-assets/tarot/concepts/'))).toBe(true)
    expect(report.assets.some((asset) => asset.category === 'source-only' && asset.path.startsWith('public/'))).toBe(false)
